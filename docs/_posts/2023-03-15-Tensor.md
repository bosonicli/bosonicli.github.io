...
Assume one consider Field at position \\( \vec{r} \\) with Source \\( q \\) at \\( \vec{R} \\), a Translational Invariant field \\( v \\) satisfies

$$
\begin{aligned}
    v( \vec{r} + \vec{-R} ) &= v(\vec{-R}) - \frac{1}{1!} \Sigma_{\alpha} r_{\alpha} v_{\alpha}(\vec{-R}) + \frac{1}{2!} \Sigma_{\alpha\beta} r_{\alpha} r_{\beta} v_{ \alpha\beta}(\vec{-R}) + \cdots   \\
    v_{ \alpha}(\vec{-R}) &= \frac{ \partial v( \vec{r} + \vec{-R} ) }{ \partial \vec{r_{\alpha}} } \vert_{\vec{r}=0} \\
    v_{ \alpha\beta}(\vec{-R}) &= \frac{ \partial^{2} v( \vec{r} + \vec{-R} ) }{ \partial \vec{r_{\alpha}} \partial \vec{r_{\beta}} } \vert_{\vec{r}=0}
\end{aligned}
$$
